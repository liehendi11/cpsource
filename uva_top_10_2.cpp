...
{
	int wordX = findLowerBound(x);
	int wordY = findLowerBound(y);

	if (dict[wordX].length() < dict[wordY].length()) return true;
	else
		if ((dict[wordX].length() == dict[wordY].length()))
		{
			if (getRank(x) < getRank(y)) return true;
			if ((getRank(x) == getRank(y)) && (getRank(x+k) < getRank(y+k))) return true;
			if ((getRank(x) == getRank(y)) && (getRank(x+k) == getRank(y+k)) && (wordX<wordY)) return true;		
		}
	return false;
}
